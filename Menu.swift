...
class Menu: UIViewController,UITextFieldDelegate,UITabBarDelegate {
    ...
    func tagMax(choise:Int)->Int{
        var tagMax = 0
        tagMax = appDelegate.data[choise].count
        return (tagMax)
    }
    // レフトスワイプ時に実行される
    ...
    override func viewDidLoad() {
        ...
        //写真ボタン作成
        for d in 0..<appDelegate.data[tagFlag1][appDelegate.tagFlag2].count{
            var e = 0
            if d <= 2{e=0}else{e=1}
            self.view.addSubview(button.make(xv:30+(220*d)-(660*e),yv:225+(220*e),wv:200,hv:200,f:20,b:appDelegate.data[tagFlag1][appDelegate.tagFlag2][d].pic,c:0+d,d:0,e:1,m:0))
        }
        //共通ボタン作成
        ...
        for d in 0..<appDelegate.data[tagFlag1].count{
            self.view.addSubview(button.make(xv:5+(115*d),yv:90,wv:110,hv:70,f:25,b:appDelegate.tag2[tagFlag1][d],c:25+d,d:1,e:0,m:1))
        }
        //tag2 選択ラベル
    ...
    @objc func selection(sender: UIButton){
        ...
        switch sender.tag{
        case k://写真ボタンを押した時
            appDelegate.st(sn:appDelegate.data[tagFlag1][appDelegate.tagFlag2][k].name,pn:appDelegate.data[tagFlag1][appDelegate.tagFlag2][k].pic)
            loadView()
            viewDidLoad()
            audioPlayerInstance.play()
        case 4:
            self.present(view.viewSet(open: Order(),anime: .flipHorizontal), animated: false, completion: nil)
            audioPlayerInstance.play()
        case 50://戻る
            //tag2の初期化
