...
class ViewController: UIViewController ,selection{
    ...
    var addTimer = Timer()
    let menuArray = [["set","ikura","osusume"],["nigiri1","nigiri2","gunkan"],["side","deza","drink"],["開発者モード","会計確認   　注文履歴","店員呼出"]]
    ...
    override func viewDidLoad() {
        ...
        for i in 0...3{
            for j in 0...2{
                if i == 3 {
                    self.view.addSubview(button.make(xv:80 + j * 300,yv:170 + i * 140,wv:280,hv:130,f:20,b:menuArray[i][j],c: i * 3 + j,t:1))
                }else{
    ...
    @objc func selection(sender: UIButton){
        ...
        switch sender.tag{
            case 9:
                ...
                appDelegate.window?.rootViewController = Test()// rootViewControllerに入れる
                appDelegate.window?.makeKeyAndVisible() // 表示
                audioPlayerInstance.play()
            case 10:
                addTimer.invalidate()
                self.present(view.viewSet(open: History(),anime: .flipHorizontal), animated: false, completion: nil)
                audioPlayerInstance.play()
            case 11:
                addTimer.invalidate()
                self.present(view.viewSet(open:  Call(),anime: .flipHorizontal), animated: false, completion: nil)
                audioPlayerInstance.play()
            case 20://動画再生中のタップ
                addTimer.invalidate()
            ...
            case k://メニュー
                ...
                addTimer.invalidate()
                self.present(view.viewSet(open: Menu(),anime: .flipHorizontal), animated: false, completion: nil)
                audioPlayerInstance.play()
            default:break
        }
    }
