        ...
        switch appDelegate.viewType{
            ...
            case "realm":
                return[Section(title: "guestData", items: ["入室時間", "退出時間","大人","子供","席タイプ","皿数"]),
                       Section(title:"appSetting",items:["二次元コード","タッチ音","スクリーンセーバ",])]
                        //sections[0].count=3,sections[2][0].items.count=1
            default:
            return[Section(title: "アカウント", items: ["席番号", "到着時間"])]//sections[0].count=1
        }
    }
...
extension SplitViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if sectionCount > 1 {
            ...
            let bezierPath = UIBezierPath(roundedRect:cell.bounds.insetBy(dx: 0.0, dy: 2.0),cornerRadius: cornerRadius)
            shapeLayer.path = bezierPath.cgPath
            cell.layer.mask = shapeLayer
        }
