...
class order: UIViewController,UITextFieldDelegate,UITabBarDelegate {
    ...
    @objc func B1(sender: UIButton){
        ...
        case 18://注文
            dishSum += box[0].qty + box[1].qty + box[2].qty
            appDelegate.qr_string = "注文は\(dishSum)皿で会計金額は\(dishSum * 110)円です"
            //realm
            let realm = try! Realm()
            let dishCount = realm.objects(r_data3.self).last
            try! realm.write {
                dishCount!.dish = dishSum
            }
